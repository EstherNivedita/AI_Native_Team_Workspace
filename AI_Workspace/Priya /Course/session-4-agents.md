...
This is the main agent file — the orchestrator. Guide them to write a prompt that:
...
A focused agent for design critique. It reads context/brand-guidelines.md and context/design-system.md, applies skills/design-review.md, and outputs a structured review with scores and specific feedback per criteria.

Have them actually test it — give it a description of a recent design (or a screenshot if available) and see the structured critique it produces. Ask: "Is this feedback useful? What would you change about the criteria? What's missing?" Iterate the skill file based on their feedback. This teaches them that agents are living systems you refine, not set-and-forget.

### Build: Create tasks/collateral-creator-sub-agent.md
A focused agent for generating collateral briefs. It reads context/brand-guidelines.md, pulls from references/marketing-collateral/, applies skills/brand-compliance-check.md, and generates a structured brief for a specific collateral piece — dimensions, copy direction, visual approach, brand compliance notes.
